...
export function QuoteForm({ customers, products, nextQuoteNo }: { customers: CustomerOption[]; products: ProductOption[]; nextQuoteNo: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/panel/quotes", {
      method: "POST",
    ...
    if (!response.ok) {
      setError(result?.error ?? "Teklif oluşturulamadı.");
      setBusy(false);
      return;
    }

    setMessage("Teklif oluşturuldu.");
    setQuoteNo(result?.data?.nextQuoteNo ?? quoteNo);
    setNote("");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <label className="space-y-2"><span className="text-sm font-semibold text-slate-600">Teklif no</span><input value={quoteNo} onChange={(e)=>setQuoteNo(e.target.value)} required /></label>
        ...
        <label className="space-y-2"><span className="text-sm font-semibold text-slate-600">Geçerlilik</span><input type="date" value={validUntil} onChange={(e)=>setValidUntil(e.target.value)} /></label>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 xl:col-span-4"><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Toplam</p><p className="mt-2 text-2xl font-black text-slate-900">{new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY" }).format(preview.grandTotal)}</p><p className="mt-1 text-xs text-slate-500">Ara toplam {new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY" }).format(preview.subtotal)} · KDV {new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY" }).format(preview.vatTotal)}</p></div>
      </div>

      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Teklif Kalemleri</p>
            <p className="mt-1 text-sm text-slate-600">Bir teklif içinde birden fazla satır planlayabilirsin.</p>
          </div>
      ...
      <label className="space-y-2 block"><span className="text-sm font-semibold text-slate-600">Not</span><textarea value={note} onChange={(e)=>setNote(e.target.value)} rows={3} placeholder="Teklif şartları, teslim planı veya özel açıklama" /></label>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button disabled={busy} className="rounded-2xl bg-[var(--brand)] px-5 py-3 text-sm font-black text-white disabled:opacity-60">{busy ? "Kaydediliyor..." : "Teklif Oluştur"}</button>
      </div>
    </form>
  );
}
