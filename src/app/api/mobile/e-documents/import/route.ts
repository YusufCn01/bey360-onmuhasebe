...
async function findExistingSupplier(tenantId: string, parsed: ParsedIncomingDocument) {
  if (parsed.supplierTaxNumber) {
    const byTaxNumber = await db.supplier.findFirst({ where: { tenantId, taxNumber: parsed.supplierTaxNumber } });
...
export async function POST(request: NextRequest) {
  ...
  if (!appType || !uuid) {
    return NextResponse.json({ success: false, error: { message: "Belge bilgisi eksik." } }, { status: 422 });
  }

  if (![1, 4].includes(appType)) {
    return NextResponse.json({ success: false, error: { message: "Şimdilik sadece gelen faturalar ve gelen irsaliyeler içe alınabilir." } }, { status: 422 });
  }

  const settings = await db.eInvoiceSettings.findUnique({ where: { tenantId: context.tenant.id } });
  if (!settings) {
    return NextResponse.json({ success: false, error: { message: "Önce Hızlı Bilişim ayarlarını kaydetmelisiniz." } }, { status: 422 });
  }
  ...
  if (duplicate) {
    return NextResponse.json({ success: false, error: { message: `Bu belge daha önce içe alınmış: ${duplicate.invoiceNo}` } }, { status: 409 });
  }
  ...
  try {
    ...
    const productResolution = await Promise.all(
      parsed.items.map(async (item) => ({ item, existingProduct: await findExistingProduct(context.tenant.id, item) })),
    );
    ...
    if (!existingSupplier && !createMissingSupplier) {
      return NextResponse.json({ success: false, error: { message: "Tedarikçi bulunamadı. İçe almadan önce tedarikçi oluşturmanız gerekiyor." }, data: analysis }, { status: 409 });
    }

    if (missingProducts.length > 0 && !createMissingProducts) {
      return NextResponse.json({ success: false, error: { message: "Belgedeki bazı ürünler sistemde bulunamadı. İçe almadan önce ürünleri oluşturmanız gerekiyor." }, data: analysis }, { status: 409 });
    }
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const invoice = await tx.invoice.create({
        data: {
          tenantId: context.tenant.id,
          branchId: context.membership.branchId ?? firstBranch?.id ?? null,
          supplierId,
          invoiceNo,
          direction: InvoiceDirection.PURCHASE,
